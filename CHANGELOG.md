...
- 新增「游戏模式（CS2）」：交火时自动暂停或压低音乐，阵亡观战 / 候场 / 菜单时自动恢复播放。通过 Valve 官方 Game State Integration (GSI) 本地接入，只读官方对局数据、不读游戏内存，反作弊安全；视觉控制台「游戏模式」分区可一键安装 CS2 集成、切换「完全暂停 / 压低音量」、开关「观战播放歌曲」。
- 搜索新增「YouTube」标签：接通普通 YouTube 视频搜索，可播放 YouTube Music 曲库未收录、但 YouTube 上有的歌曲（走同一套音频管线，播放无需额外配置）。
- 修复应用拆分重构后 Electron 启动挂起、窗口出不来的问题：`desktop/main.js` 现在会在 require `server.js` 后显式启动本地服务器。
- 游戏模式开关加入初始化兜底，避免控制器未就绪时点击无响应。

## v1.3.5

- 增强歌曲律动镜头：画面震动会根据低频鼓点、drop/downbeat/accent 和歌曲高潮段落提高强度与触发频率。
- 新增独立 `beat-dynamics` 前端模块，专门计算高潮鼓点的镜头幅度、触发间隔和视觉脉冲，降低后续调律动时影响队列、更新、壁纸等模块的风险。
- 补充律动模块静态回归测试，确认高潮鼓点比弱拍拥有更强镜头推拉、更短触发间隔和更高视觉脉冲。

## v1.3.4

- 将搜索播放后的推荐队列逻辑拆成独立控制器，搜索结果开播会固定以第一首歌作为 seed 生成后续推荐，减少“只有当前一首歌”的偶发状态。
- 继续拆分 `public/index.html`：首页发现 tile、歌单详情视图、更新面板文案/进度状态迁移到独立模块，降低壁纸、队列、更新和首页推荐之间互相影响的风险。
- 快速补丁更新增加写入失败回滚，补丁应用中途失败时会恢复已经备份的文件，避免半更新状态。
- 新增搜索推荐队列 smoke、主页面壁纸切换 smoke，并保留透明闪屏防回归检查。
...
- 修复 YouTube Music 歌单创建与收藏写入：新建歌单、添加歌曲不再返回假成功，改为调用真实 YTM 歌单接口。
- 首页 Library 卡片改为「点赞的歌曲」，并改用真实 YouTube Music 收藏数量，不再固定显示 100 首。
- 首页新增「常听推荐」一行，会根据最近播放和常听歌手整理推荐。
- 搜索播放歌曲后，左侧队列会直接以第一首歌为种子生成推荐队列；radio 返回后按种子歌身份落队列，并加入短重试，避免偶发只剩当前一首。
- 队列渲染会过滤 `Unknown`、`未知歌手`、缺少 id 或封面的占位歌曲，避免黑块占位项混入。
- 默认只使用 YouTube Music 音源与歌单接口，Spotify 源默认禁用，避免 Spotify 会话 403 影响搜索和队列推荐。
